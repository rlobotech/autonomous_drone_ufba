...
######################=-
	...
	# Aplica o movimento de turnAngle indo pra frente
	def moveTurnAngle(self,ndir,speed,*args):
		opos = 		self.__NavData["demo"][2][2]			# get the source/current (original) angle
		npos = 		opos+ndir								# calculate the destination (new) angle
		minaxis = 	opos									# to make sure, that the jump from -180 to 180 will...
		maxaxis = 	opos									# ...be correctly handled
		...
		while not stop and counter<=5:
			...
			self.__speed = speed
			if cpos > (npos+kalib):							# turn left, if destination angle is lower
				self.move(0.0,self.__checkSpeedValue(speed),0.0,-1)
				if direction   ==  0:	direction = -1
				elif direction ==  1:	direction, counter = -1, counter+1
			else:											# turn right if destination angle is higher
				self.move(0.0,self.__checkSpeedValue(speed),0.0,1)
	...
	def moveAltitude(self,altitude,speed,*args):
		accurateness = 0		
		try:	accurateness = args[0]
		except:	pass
		if accurateness<=0:
			accurateness = 0.10					# Destination UpDown can differ +/- this value in CM
		stop =		False
		while not stop:
			ndc = self.__NavDataCount						# wait for the next NavData-package
			while ndc == self.__NavDataCount:		time.sleep(0.001)
			current = (self.__NavData["demo"][3] / 100)
                	if altitude > (current + accurateness):
				self.move(0.0,speed,1,0.0)
                        elif altitude < (current - accurateness):
				self.move(0.0,speed,-1,0.0)
			else:
				self.stop()
				time.sleep(0.01)
				stop = True
                                print "altitude atual: ", current
                self.stop()
                time.sleep(0.01)
		self.move(0.0,self.__checkSpeedValue(speed),0.0,0.0)
		return(True) 
